...
namespace Blog.Controllers
{
    public class CategoryController : BaseController
    {
        ...
        public ActionResult SubCategory(int id)
        {
            ...
            var posts = subCategory.Posts.Where(a => a.postType == Objects.PostType.Sqr && a.Translations.Any(x => x.languageId == currentCultureName && (x.Description != null))).OrderByDescending(a => a.PostedOn).Take(20).ToList();
            ViewBag.CategoryName = subCategory.Name;
            ViewBag.id = subCategory.Id;
            return View(posts);

        }
        public ActionResult More(int id, int subId, int page = 1)
        {
            string currentCultureName = CultureInfo.CurrentCulture.Name.Substring(0, 2);
            var category = db.Categories.Find(id);
